...
const STORAGE_KEY = "classic-fruits-game";
const STARTING_JACKPOT = 5000;
const MAX_REGULAR_WIN_MULTIPLIER = 10;
...
export default function ClassicFruits({
  player,
  onCreditsChange,
}) {
...
const firstSaveRef = useRef(true);
  ...
  useEffect(() => {
    return () => {
      ...
      reelTimeoutsRef.current.forEach((timeoutId) =>
  ...
  const westernNotes = [740, 830, 930, 1046, 1174];

  playTone({
    frequency: westernNotes[reelIndex] || 880,
    duration: 0.1,
    volume: 0.15,
    type: "sine",
  });
...
function playWinSound(bigWin = false) {
  ...
  function playWildSound() {
    // Dos golpes secos tipo disparo, creados con Web Audio.
    playTone({
      frequency: 125,
      duration: 0.1,
      volume: 0.22,
...
const regularPrize = Math.min(
  linePrize + scatterWin.amount,
  bet * MAX_REGULAR_WIN_MULTIPLIER
);

const totalPrize =
  regularPrize + jackpotPrize;
    ...
    if (jackpotPrize > 0) {
      resultMessage =
        `🔔 ¡FRUIT JACKPOT! ${totalPrize} CRÉDITOS`;
    } else if (
      lineWins.length > 0 &&
      ...
      const ratio = totalPrize / bet;

      resultMessage =
        ratio >= 8
          ? `💥 ¡GRAN PREMIO! ${totalPrize} CRÉDITOS`
          : ratio >= 4
            ? `🍇 ¡PREMIO! ${totalPrize} CRÉDITOS`
...
setCredits(creditsAfterBet);
onCreditsChange?.(creditsAfterBet);
      setJackpot((current) =>
    ...
    for (
      ...
      const timeoutId = setTimeout(() => {
        ...
        reelTimeoutsRef.current.push(bounceTimeoutId);
        ...
        if (index === COLUMNS - 1) {
          ...
          const flatResult = finalResult.flat();
          const wildCount = flatResult.filter((symbol) => symbol === WILD).length;
          const scatterCount = flatResult.filter((symbol) => symbol === SCATTER).length;
...
if (prize.amount > 0) {
          const creditsAfterPrize =
  credits - (isFreeSpin ? 0 : bet) + prize.amount;

setCredits(creditsAfterPrize);
onCreditsChange?.(creditsAfterPrize);
}
  ...
  try {
    ...
    console.error("Error al guardar la jugada de Classic Fruits:", error);
  }
})();
        }
      }, stopTime);
  ...
  return (
    <main className="page classic-fruits-page">
      <section
        className={[
          "machine",
      ...
      >
        ...
        <div className="western-heading" aria-label="Classic Fruits">
          <span className="western-kicker">LA FRONTERA TE ESPERA</span>
          <h1 className="title">CLASSIC FRUITS</h1>
          <span className="western-stars" aria-hidden="true">
            ★ ☆ ★ 🔔 ★ ☆ ★
          </span>
        </div>

        <p className="subtitle">
          FRUIT · ORO · GIROS GRATIS
        </p>

        <div className="jackpot-panel">
          <span>🔔 FRUIT JACKPOT</span>
          <strong>{jackpot.toLocaleString("es-AR")}</strong>
          <small>5 sheriffs en línea</small>
        </div>
        ...
        <div className="bottom-buttons">
          ...
          <div className="saved-message">
            💾 Partida Classic Fruits guardada
          </div>
        </div>
            ...
            <div className="paytable-card" onClick={(event) => event.stopPropagation()}>
              ...
              <div className="paytable-grid">
                ...
                <div className="paytable-row scatter-pay">
                  ...
                  <span>4 = ×5 + 10 gratis</span>
                  <span>5+ = ×10 + 15 gratis</span>
                </div>
              </div>
              <small>🔔 Cinco campanas en línea entregan el Botín del Campana.</small>
            </div>
          </button>
        )}
