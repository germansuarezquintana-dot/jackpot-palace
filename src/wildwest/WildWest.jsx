...
const STORAGE_KEY = "wild-west-game";
...
const totalPrize =
  regularPrize + jackpotPrize;
    ...
    if (jackpotPrize > 0) {
      resultMessage =
        `🤠 ¡BOTÍN DEL SHERIFF! ${totalPrize} CRÉDITOS`;
    } else if (
      lineWins.length > 0 &&
      ...
      resultMessage =
        ratio >= 8
          ? `💥 ¡DUELO GANADO! ${totalPrize} CRÉDITOS`
          : ratio >= 4
            ? `💰 ¡RECOMPENSA! ${totalPrize} CRÉDITOS`
            : `⭐ BUENA TIRADA · ${totalPrize} CRÉDITOS`;
    } else if (lineWins.length > 1) {
      resultMessage =
...
if (prize.amount > 0) {
  ...
  try {
    ...
    console.error("Error al guardar la jugada de Wild West:", error);
  }
})();
        }
      }, stopTime);
  ...
  return (
    <main className="page wild-west-page">
      <section
        className={[
          "machine",
      ...
      >
        ...
        <div className="western-heading" aria-label="Wild West">
          <span className="western-kicker">LA FRONTERA TE ESPERA</span>
          <h1 className="title">WILD WEST</h1>
        ...
        <p className="subtitle">
          SHERIFF · ORO · GIROS GRATIS
        </p>

        <div className="jackpot-panel">
          <span>🤠 BOTÍN DEL SHERIFF</span>
        ...
        {paytableOpen && (
          ...
          >
            <div className="paytable-card" onClick={(event) => event.stopPropagation()}>
              ...
              <small>🤠 Cinco sheriffs en línea entregan el Botín del Sheriff.</small>
            </div>
          </button>
        )}
