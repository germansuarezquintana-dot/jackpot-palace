...
const STORAGE_KEY = "pirate-gold-game";
...
export default function PirateGold({
  player,
  onCreditsChange,
}) {
...
const firstSaveRef = useRef(true);
  ...
  const pirateNotes = [740, 830, 930, 1046, 1174];

  playTone({
    frequency: pirateNotes[reelIndex] || 880,
    duration: 0.1,
    volume: 0.15,
    type: "sine",
  });
...
const totalPrize =
  regularPrize + jackpotPrize;
    ...
    if (jackpotPrize > 0) {
      resultMessage =
        `☠️ ¡PIRATE JACKPOT! ${totalPrize} CRÉDITOS`;
    } else if (
      lineWins.length > 0 &&
      ...
      resultMessage =
        ratio >= 8
          ...
          : ratio >= 4
            ? `💎 ¡TESORO! ${totalPrize} CRÉDITOS`
            : `⚓ ¡GANASTE! · ${totalPrize} CRÉDITOS`;
    } else if (lineWins.length > 1) {
      resultMessage =
        `☠️ ${lineWins.length} LÍNEAS GANADORAS · ` +
        `BOTÍN ${totalPrize}`;
    }
...
onCreditsChange?.(creditsAfterPrize);
  ...
  try {
    ...
    console.error("Error al guardar la jugada de Pirate Gold:", error);
  }
})();
        }
      }, stopTime);
  ...
  return (
    <main className="page pirate-gold-page">
      <section
        className={[
          "machine",
      ...
      >
        ...
        <div className="western-heading" aria-label="Pirate Gold">
          <span className="western-kicker">EL TESORO TE ESPERA</span>
          <h1 className="title">PIRATE GOLD</h1>
        ...
        <p className="subtitle">
          PIRATAS · TESOROS · GIROS GRATIS
        </p>

        <div className="jackpot-panel">
          <span>☠️ PIRATE JACKPOT</span>
          <strong>{jackpot.toLocaleString("es-AR")}</strong>
          <small>5 calaveras en línea</small>
        </div>
        ...
        {paytableOpen && (
          ...
          >
            <div className="paytable-card" onClick={(event) => event.stopPropagation()}>
              ...
              <small>🏴‍☠️ Cinco piratas en línea entregan el Pirate Jackpot.</small>
            </div>
          </button>
        )}
